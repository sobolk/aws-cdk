...
console.log('\n✨  Testapp001 import \'source-map-support/register\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
import * as cdk from 'aws-cdk-lib';
singleImportElapsedTime = new Date().getTime() - singleImportStartTime;
console.log('\n✨  Testapp001 import * as cdk from \'aws-cdk-lib\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
import { Testapp001Stack } from '../lib/testapp001-stack';
singleImportElapsedTime = new Date().getTime() - singleImportStartTime;
console.log('\n✨  Testapp001 import { Testapp001Stack } from \'../lib/testapp001-stack\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
const importElapsedTime = new Date().getTime() - importStartTime;
console.log('\n✨  Testapp001 all import: %ss\n', importElapsedTime/1000);
...
process.on('exit', () => {
    ...
    console.log('\n✨  Testapp001 total processing time: %ss\n', globalElapsedTime/1000);
    console.log('\n✨  Testapp001 processing time no imports: %ss\n', noImportsElapsedTime/1000);
});

export const app = new cdk.App();
new Testapp001Stack(app, 'Testapp001Stack', {
