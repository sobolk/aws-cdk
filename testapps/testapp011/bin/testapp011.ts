...
console.log('\n✨  Testapp011 import \'source-map-support/register\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
import * as cdk from 'aws-cdk-lib';
singleImportElapsedTime = new Date().getTime() - singleImportStartTime;
console.log('\n✨  Testapp011 import * as cdk from \'aws-cdk-lib\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
import { Testapp011Stack } from '../lib/testapp011-stack';
singleImportElapsedTime = new Date().getTime() - singleImportStartTime;
console.log('\n✨  Testapp011 import { Testapp011Stack } from \'../lib/testapp011-stack\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
const importElapsedTime = new Date().getTime() - importStartTime;
console.log('\n✨  Testapp011 all import: %ss\n', importElapsedTime/1000);
...
process.on('exit', () => {
    ...
    console.log('\n✨  Testapp011 total processing time: %ss\n', globalElapsedTime/1000);
    console.log('\n✨  Testapp011 processing time no imports: %ss\n', noImportsElapsedTime/1000);
});

const app = new cdk.App();
new Testapp011Stack(app, 'Testapp011Stack', {
