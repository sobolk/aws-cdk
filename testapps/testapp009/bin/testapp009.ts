...
console.log('\n✨  Testapp009 import \'source-map-support/register\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
import * as cdk from 'aws-cdk-lib';
singleImportElapsedTime = new Date().getTime() - singleImportStartTime;
console.log('\n✨  Testapp009 import * as cdk from \'aws-cdk-lib\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
import { Testapp009Stack } from '../lib/testapp009-stack';
singleImportElapsedTime = new Date().getTime() - singleImportStartTime;
console.log('\n✨  Testapp009 import { Testapp009Stack } from \'../lib/testapp009-stack\': %ss\n', singleImportElapsedTime/1000);
singleImportStartTime = new Date().getTime();
const importElapsedTime = new Date().getTime() - importStartTime;
console.log('\n✨  Testapp009 all import: %ss\n', importElapsedTime/1000);
...
process.on('exit', () => {
    ...
    console.log('\n✨  Testapp009 total processing time: %ss\n', globalElapsedTime/1000);
    console.log('\n✨  Testapp009 processing time no imports: %ss\n', noImportsElapsedTime/1000);
});

const app = new cdk.App();
new Testapp009Stack(app, 'Testapp009Stack', {
